...
test.describe("API Verification examples", () => {
  ...
  test("Verify multiple records returned against store static response", async ({ request }) => {
  ...
  test("Verify PUT request", async ({request}) => {
    ...
    const response = await request.put('https://reqres.in/api/user/1', {
      headers: { 'x-api-key': 'reqres-free-v1' },
      data: updateUser
    })
  ...
  test('Verify user is deleted', async ({request}) => {
    const response = await request.delete('https://reqres.in/api/user/1', {
      headers: { 'x-api-key': 'reqres-free-v1' }
    })
